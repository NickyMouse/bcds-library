...
public class BookInfoBuilder extends BaseBuilder {
	...
	protected void beforeBuild() {
		...
		String info;
		if (this.isIncrementBuild()) {
			info = "[Search Builder]:Increment Build Start at "
					+ processStartTime;
		} else {
	...
	protected void fullBuild() throws IOException {
		...
		DocumentFactory factory = DocumentFactory.getInstance(
				BuildBookSearch.class, handler);
		IndexWriter writer = null;
		try {
			writer = new IndexWriter(this.getDestination(), new MMAnalyzer(),
	...
	protected void incrementBuild() throws IOException {
		...
		IndexWriter writer = null;
		try {
			Date modifiedStartTime = new Date(modifiedEndTime.getTime()
					- interval);
			List<BuildBookSearch> bookList = BookSearchServiceLocator
		...
		try {
			for (BuildBookSearch book : bookList) {
				reader.deleteDocuments(new Term(
						BookSearchConstrains.BOOK_INFO_ID, book.getBookInfoId()
								.toString()));
			}
		} catch (IOException e) {
			logger.error(e);
