...
public class BookItemDaoImpl extends SqlMapClientDaoSupport implements
	...
	public Integer insert(BookItem bookItemDO) {
		return (Integer) getSqlMapClientTemplate().insert("BOOK_ITEM.insert",
				bookItemDO);
	}
	...
	public int updateById(BookItem bookItemDO) {
		return getSqlMapClientTemplate().update("BOOK_ITEM.updateById",
				bookItemDO);
	}
	...
	public void addBookItem(final String loginId, final String isbn) {
		getSqlMapClientTemplate().insert(
				...
				new ParameterMap<String, String>().addParameter("loginId",
						loginId).addParameter("isbn", isbn));
	}
	...
	public List<BookItem> listBookItemsByLoginId(String loginId) {
		return getSqlMapClientTemplate().queryForList("listBookItemsByLoginId",
				loginId);
	 ...
	 */
	@Override
	public List<BookItem> listBookItemsByLoginIdAndState(String loginId,
			String state, int page, int pagesize) {
		return getSqlMapClientTemplate().queryForList(
				"BOOK_ITEM.listBookItemsByLoginIdAndState",
				new ParameterMap<String, Object>().addParameter("loginId",
						loginId).addParameter("state", state).addParameter(
						"skipRows", (page - 1) * pagesize).addParameter(
						"pageSize", pagesize));
	}
	...
	public List<BookItem> listBookItemByBookInfoId(int bookInfoId) {
		return getSqlMapClientTemplate().queryForList(
				"BOOK_ITEM.listBookItemByBookInfoId", bookInfoId);
	}
	 ...
	 */
	@Override
	public List<BookItem> listLendedBookItemBySubscriber(String subscriber,
			int page, int pagesize) {
		return getSqlMapClientTemplate().queryForList(
				"BOOK_ITEM.listLendedBookItemBySubscriber",
				new ParameterMap<String, Object>().addParameter("subscriber",
						subscriber).addParameter("skipRows",
						(page - 1) * pagesize).addParameter("pageSize",
						pagesize));
	}
	 ...
	 */
	@Override
	public List<BookItem> listReservatedBooksBySubscriber(String subscriber,
			int page, int pagesize) {
		return getSqlMapClientTemplate().queryForList(
	...
	public int countBookItemsByLoginIdAndState(String owner, String state) {
		return (Integer) getSqlMapClientTemplate().queryForObject(
				...
				new ParameterMap<String, String>().addParameter("loginId",
						owner).addParameter("state", state));
	}
