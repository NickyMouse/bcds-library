...
// 一、<input name=txt><input type=button value=setday
// onclick="setday(this,document.all.txt)">
...
function setday(tt,obj) // 主调函数
{
	...
	/*
	 * //判断初始化时是否使用时间,严格验证 var tmpReg = /^(\d+)-(\d{1,2})-(\d{1,2})
	 * (\d{1,2}):(\d{1,2}):(\d{1,2})/; var tmpR =outObject.value.match(tmpReg);
	 * if (tmpR!=null) { tmpR[2]=tmpR[2]-1; var tmpD=new
	 * Date(tmpR[1],tmpR[2],tmpR[3],tmpR[4],tmpR[5],tmpR[6]);
	 * if(tmpD.getFullYear()==tmpR[1] && tmpD.getMonth()==tmpR[2] &&
	 * tmpD.getDate()==tmpR[3] && tmpD.getHours()==tmpR[4] &&
	 * tmpD.getMinutes()==tmpR[5] && tmpD.getSeconds()==tmpR[6]) {
	 * bUseTime=true; meizzWriteHead(meizzTheYear,meizzTheMonth); showLayer();
