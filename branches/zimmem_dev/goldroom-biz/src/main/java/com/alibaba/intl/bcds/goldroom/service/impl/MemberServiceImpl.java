...
public class MemberServiceImpl implements MemberService {
    ...
    public Result approveMembers(List<Integer> ids) {
        try {
            ...
            for (Member member : members) {
                ...
                this.integralDao.insert(new IntegralQuery(member.getLoginId(), 100));
            }
    ...
    public List<MemberDTO> listMemberByStatus(MemberEnableEnum memberEnable) {
        ...
        List<Member> members = memberDAO.listMemberByStatus(memberEnable.getValue());
        List<Integer> memberIds = new ArrayList<Integer>();
        for (Member member : members) {
            memberIds.add(member.getId());
        }
        Map<Integer, MemberRole> roleMap = memberRoleDAO.mapMemberRoleByMemberIds(memberIds);
        for (Member member : members) {
