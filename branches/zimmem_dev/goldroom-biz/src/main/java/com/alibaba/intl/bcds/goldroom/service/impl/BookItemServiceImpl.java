...
public class BookItemServiceImpl implements BookItemService {
	...
	public void addBookItem(BookItem bookItem) {
		...
		if (modifiedRows > 0) {
			logger.info("Successfully addBookItem integral 50, "
					+ bookItem.getLoginId());
		} else {
			logger.warn("Failed to addBookItem integral 50, "
					+ bookItem.getLoginId());
		}
	}
	 ...
	 */
	@Override
	public Result listBookItemsByLoginIdAndState(String loginId, String state,
			int page, int pagesize) {

		Map<String, Object> returnMap = new HashMap<String, Object>(2);
		returnMap.put("totalCount", bookItemDao
				.countBookItemsByLoginIdAndState(loginId, state));
		returnMap
				.put("bookItemList", bookItemDao
						.listBookItemsByLoginIdAndState(loginId, state, page,
								pagesize));
	 ...
	 */
	@Override
	public Result listLendedBookItemBySubscriber(String ownerLoginID, int page,
			int pagesize) {
		Map<String, Object> returnMap = new HashMap<String, Object>(2);
		returnMap.put("totalCount", bookItemDao
				.countLendedBookItemBySubscriber(ownerLoginID));
		returnMap.put("bookItemList", bookItemDao
				.listLendedBookItemBySubscriber(ownerLoginID, page, pagesize));
		return new Result(true, null, returnMap);
	}
	 ...
	 */
	@Override
	public Result listReservatedBooksBySubscriber(String loginId, int page,
			int pagesize) {

		Map<String, Object> returnMap = new HashMap<String, Object>(2);
		returnMap.put("totalCount", bookItemDao
				.countReservatedBooksBySubscriber(loginId));
		returnMap.put("bookItemList", bookItemDao
	 ...
	 */
	@Override
	public Result reserve(String subscriber, int bookItemId, Date lendTime,
			Date returnTime) {
		...
		if (item.getState().equals(BookItem.STATE_IDLE)) {
			if (reservationDAO.updateStateByBookItemId(bookItemId,
					Reservation.STATE_TO_BE_COMFIRM) == 0) {
				...
				Member subcriber = memberInfoCache.getMemberInfo(reservation
						.getSubscriber());
				BookInfo bookInfo = bookInfoDao.findById(item.getBookInfoId());
				ReservationEmailInfo emailInfo = new ReservationEmailInfo(
			...
			int modifiedRows = this.integralDao.increaseIntegral(integralQuery);
			if (modifiedRows > 0) {
				logger.info("Successfully reserve integral 10, "
						+ item.getLoginId());
			} else {
	...
	public Result lend(int reservationId, String currentUser) {
		...
		if (bookItem.getState().equals(BookItem.STATE_RESERVATED)) {
			...
			Member subcriber = memberInfoCache.getMemberInfo(lending
					.getSubscriber());
			BookInfo bookInfo = bookInfoDao.findById(bookItem.getBookInfoId());
			GetBookEmailInfo emailInfo = new GetBookEmailInfo(owner, subcriber,
					bookInfo, lending);
			sendMailService.sendMail(emailInfo);
