...
public class RoleDAOImpl extends SqlMapClientDaoSupport implements RoleDAO {
    ...
    public int deleteByPrimaryKey(Integer id) throws SQLException {
        Role key = new Role();
        key.setId(id);
        int rows = getSqlMapClientTemplate().delete("ROLE.deleteByPrimaryKey", key);
        return rows;
    }
    ...
    public Role selectByPrimaryKey(Integer id) throws SQLException {
        Role key = new Role();
        key.setId(id);
        Role record = (Role) getSqlMapClientTemplate().queryForObject("ROLE.selectByPrimaryKey", key);
        return record;
    }
